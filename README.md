# AD-SecretGen — derive AD password hashes & Kerberos keys
...
It's a single self-contained [PEP 723](https://peps.python.org/pep-0723/) file. `uv` reads the inline dependency metadata and fetches `pycryptodome` + `rich` automatically — **including straight from the URL**, nothing to clone or install:
...
`uv` caches by URL, so add `--refresh` to pull a newer `main`. Or grab the file once:
...
To put `ad-secretgen` (and the short alias `adsg`) on your PATH:
